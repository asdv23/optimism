...
func (s *L2Sequencer) ActL2StartBlock(t Testing) {
	...
	go func() {
		s.ActL2BuildBlock(t, func() {
			ready <- struct{}{}
			<-s.sealBlockSignal
		})
		s.sealBlockDoneSignal <- struct{}{}
	}()
	<-ready
}
...
func (s *L2Sequencer) ActL2EndBlock(t Testing) {
	...
	s.sealBlockSignal <- struct{}{}
	<-s.sealBlockDoneSignal
	s.l2Building.Store(false)
}
