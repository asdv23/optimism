...
func (info *L1BlockInfo) unmarshalBinaryIsthmus(data []byte) error {
	...
	var err error
	if _, err := solabi.ReadAndValidateSignature(r, []byte(L1InfoFuncIsthmusSignature)); err != nil {
		return err
	}
	if err := binary.Read(r, binary.BigEndian, &info.BaseFeeScalar); err != nil {
...
func (info *L1BlockInfo) unmarshalBinaryJovian(data []byte) error {
	...
	var err error
	if _, err := solabi.ReadAndValidateSignature(r, []byte(L1InfoFuncJovianSignature)); err != nil {
		return err
	}
	if err := binary.Read(r, binary.BigEndian, &info.BaseFeeScalar); err != nil {
